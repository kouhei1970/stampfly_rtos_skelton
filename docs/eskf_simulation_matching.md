...
PCリプレイを実装・修正する際のチェックリスト:

- [ ] 各センサー更新が固定間隔で行われているか確認
- [ ] 更新間隔がデバイスのタスク周期と一致しているか確認
- [ ] データ変化検出ではなくカウンタベースで更新しているか確認
- [ ] dt（時間刻み）がデバイスと一致しているか確認（特にFlowは10ms固定）
- [ ] センサー更新の順序がデバイスと一致しているか確認
