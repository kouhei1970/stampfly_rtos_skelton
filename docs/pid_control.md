...
1. **アクチュエータ応答**: モーター+プロペラの時定数
   - ブラシレスモーター: τm ≈ 10〜50ms
   - 帯域上限: ωm ≈ 1/τm ≈ 20〜100 rad/s

2. **サンプリング周波数**: 制御周期 T による制限
   - ナイキスト: ωs/2 = π/T
   - 実用上: ωc < ωs/10 程度が望ましい
   - T = 2.5ms (400Hz) の場合: ωc < 250 rad/s

3. **センサノイズ**: 高帯域にするとノイズが乗る
   - ジャイロノイズスペクトル密度から決定

**StampFlyの場合の目安:**
- 制御周期: T = 2.5ms (400Hz)
- モーター応答: τm ≈ 20〜30ms
- 推奨帯域: ωc ≈ 30〜50 rad/s (5〜8 Hz)
